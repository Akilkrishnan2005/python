...
def login():
    ...
    try:
        conn = connect_db()
        cur = conn.cursor()
        cur.execute("SELECT * FROM login WHERE USERNAME=%s AND PASSWORD=%s", (Username, Password)) or cur.execute("SELECT * FROM sign WHERE USERNAME=%s AND PASSWORD=%s", (Username, Password))
        result = cur.fetchone()
        conn.close()
        ...
        if result:
            ...
            subprocess.Popen([sys.executable,r"C:\DATA SCIENCE\PYTHON PROJECT\VEHICLE SERVICE MANGEMENT SYSTEM\FORM PAGE.py"])
...
# Load Image 
img = Image.open(r"C:\Users\akilk\Downloads\093b16e8-0aa8-448b-ae57-704636e1c132.jpg")  
img = img.resize((660, 600))
photo = ImageTk.PhotoImage(img)

img_label = tk.Label(left_frame, image=photo, bg="#0f172a")
img_label.place(relx=0.5, rely=0.5, anchor="center")

tk.Label(left_frame,text="VEHICLE SERVICE MANAGEMENT SYSTEM",font=("Arial", 20, "bold"),fg="dodger blue",bg="gray7").pack(pady=80)
...
# ---------- Avatar Image ----------
avatar_img = Image.open(r"C:\Users\akilk\Downloads\Gemini_Generated_Image_eage02eage02eage.png")
...
tk.Label(right_frame,text="LOGIN",font=("Arial", 15,"bold"),fg="white",bg="#020617").pack(pady=10)

# ---------- Load Icons ----------
user_icon_img = Image.open(r"C:\Users\akilk\Downloads\download.png").resize((20, 20))
user_icon = ImageTk.PhotoImage(user_icon_img)

lock_icon_img = Image.open(r"C:\Users\akilk\Downloads\download (1).png").resize((20, 20))
...
entry_user = tk.Entry(user_frame,font=("Arial", 12),bg="#020617",fg="white",insertbackground="white",bd=0)
...
entry_pass = tk.Entry(pass_frame,font=("Arial", 12),bg="#020617",fg="white",insertbackground="white",bd=0,show="*")
entry_pass.pack(side="left", fill="x", expand=True)

tk.Frame(right_frame, height=1, bg="gray").pack(fill="x", padx=80, pady=(0, 20))

# ---------- Login Button ----------
tk.Button(right_frame,text="LOGIN",font=("Arial", 12, "bold"),bg="#2563eb",fg="white",bd=0,height=2,command=login).pack(fill="x", padx=80)

# ---------- Extra Links ----------
tk.Label(right_frame,text="Forgot Password?",fg="gray",bg="#020617",cursor="hand2").pack(pady=10)

tk.Label(right_frame,text="No account yet?  Sign Up Now",fg="#38bdf8",bg="#020617",cursor="hand2").pack()
